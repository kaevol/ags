...
class InteractionDataset(Dataset):
    def __init__(self,
        ...
        # Only include CSV files in raw file names
        all_files = os.listdir(self.raw_dir)
        self._raw_file_names = [f for f in all_files if f.endswith('.csv')]
        ...
        for raw_path in tqdm(self.raw_paths):
            ...
            try:
                ...
                required_columns = ['case_id', 'track_id', 'frame_id', 'timestamp_ms', 'agent_type', 
                                  'x', 'y', 'vx', 'vy', 'psi_rad', 'length', 'width']
                missing_columns = [col for col in required_columns if col not in df.columns]
                
                if missing_columns:
                    print(f"Warning: Missing columns in {raw_file_name}: {missing_columns}")
                ...
                print(f"Error reading {raw_file_name}: {str(e)}")
                continue
    ...
    def process(self) -> None:
        for raw_path in tqdm(self.raw_paths):
            ...
            raw_dir, raw_file_name = os.path.split(raw_path)
            scenario_name = os.path.splitext(raw_file_name)[0]
            map_path = os.path.join(raw_dir, scenario_name.split('_train')[0] + '.osm')
    ...
    def get_features(self, 
        ...
        # find ego agent
        ego_df = df[df['agent_type'] == 'ego']
        if len(ego_df) == 0:
            raise ValueError("No ego agent found in the data")
        ego_agent_id = ego_df['track_id'].values[0]
        agent_index = agent_ids.index(ego_agent_id)
